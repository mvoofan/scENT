...
def get_accuracy(cluster_assignments, y_true, n_clusters):
    ...
    returns:    a tuple containing the accuracy and confusion matrix,
                in that order
    '''
    y_pred, confusion_matrix, revised_confusion_matrix = get_y_preds(cluster_assignments, y_true, n_clusters)
    # calculate the accuracy
    return np.mean(y_pred == y_true), confusion_matrix, revised_confusion_matrix
'''
def style_diag(data):
...
def color_sign(matrix):
    '''
    df = pd.DataFrame( matrix, columns=[str(i) for i in range(matrix.shape[-1])]  )
    df.style.apply(style_diag, axis=None)
    print(df)
    '''
    c = colorama.Fore.GREEN if np.indices(np.shape(matrix))[0]==np.indices(np.shape(matrix))[1] else colorama.Fore.BLACK
    return f'{c}{matrix}'
...
def my_print2(matrix):
    for i in range(np.shape(matrix)[0]):
        for j in range(np.shape(matrix)[1]):
            s1=str(matrix[i,j])
            c=colorama.Fore.GREEN if i==j else colorama.Fore.WHITE
            print(c+s1,' '*(4-len(s1)),end="")
        print('\n')
    print(colorama.Fore.RESET)

def print_accuracy_to_fo(cluster_assignments, y_true, n_clusters,fo,  extra_identifier=''):
    '''
    Convenience function: prints the accuracy
    '''
    # get accuracy
    accuracy, confusion_matrix, revised_confusion_matrix = get_accuracy(cluster_assignments, y_true, n_clusters)
    # get the confusion matrix
    print('confusion matrix{}: '.format(extra_identifier), file=fo)
    print(confusion_matrix, file=fo)
    print('revised_confusion matrix{}: '.format(extra_identifier))
    print(revised_confusion_matrix, file=fo)
    print('Clustering{} accuracy: '.format(extra_identifier) + str(np.round(accuracy, 4)), file=fo)
...
def print_accuracy(cluster_assignments, y_true, n_clusters, extra_identifier=''):
    ...
    my_print2(confusion_matrix)
    print('revised_confusion matrix{}: '.format(extra_identifier))
    my_print2(revised_confusion_matrix)
    print('Clustering{} accuracy: '.format(extra_identifier) + str(np.round(accuracy, 4)))
...
def get_y_preds(cluster_assignments, y_true, n_clusters):
    ...
    confusion_matrix = sklearn.metrics.confusion_matrix(y_true, cluster_assignments, labels=[i for i in range(n_clusters)])
    # compute accuracy based on optimal 1:1 assignment of clusters to labels
    cost_matrix = calculate_cost_matrix(confusion_matrix, n_clusters)
    indices = Munkres().compute(cost_matrix)
    kmeans_to_true_cluster_labels = get_cluster_labels_from_indices(indices)
    y_pred = kmeans_to_true_cluster_labels[cluster_assignments]
    revised_confusion_matrix=confusion_matrix[kmeans_to_true_cluster_labels.astype('int8'),:]  #added
    return y_pred, confusion_matrix, revised_confusion_matrix
